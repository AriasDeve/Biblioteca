...
-- 2° List books by category
	DELIMITER $$
	CREATE PROCEDURE spu_bookscategory_list(
		IN _idsubcategorie INT
	)
	BEGIN
...
-- 4° List subcategories
	DELIMITER $$
	CREATE PROCEDURE spu_subcategories_list()
	BEGIN
		SELECT idsubcategorie, subcategoryname
			FROM subcategories;
	END $$
	
	CALL spu_subcategories_list();
...
-- 6° Edit book record
	DELIMITER $$
	CREATE PROCEDURE spu_books_obtain(IN _idbook INT)
	BEGIN
	...
	CALL spu_books_update ('1','1','1','02','Probabilidad y estadística como trabajar con niños y jóvenes','Ana P, de Bressan/Oscar Bogisic','B','Biblioteca escolar','m','m');
